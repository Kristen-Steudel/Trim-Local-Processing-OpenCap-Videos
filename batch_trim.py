...
def process_batch():
    """
    Reads the CSV and iterates through the videos, trimming each pair.
    
    CSV Format:
    Cam 1 File Location,Camera 1 Input File,Cam 2 File Location,Camera 2 Input File,Start Frame,Frame Count,[Start Frame 2],[Frame Count 2],...
    ...
    try:
        with open(INPUT_CSV_FILE, mode='r', newline='') as file:
            ...
            for row_num, row in enumerate(reader, start=2):
                ...
                # Ensure we have at least the basic 6 columns
                if len(row) < 6:
                    print(f"WARNING: Row {row_num} has fewer than 6 columns. Skipping.")
                    continue
                
                # Extract base information (first 4 columns are paths/filenames)
                ...
                # Get FPS for both cameras (should be the same, but detect independently)
                fps_1 = get_video_fps(input_path_1)
                fps_2 = get_video_fps(input_path_2)
                
                # Process all trim ranges for this video pair
                # Columns 4+ come in pairs: (Start Frame, Frame Count)
                trim_index = 1
                for i in range(4, len(row), 2):
                    ...
                    print(f"\n{'='*70}")
                    print(f"Row {row_num}, Trim Set {trim_index}: {os.path.basename(cam1_in)} & {os.path.basename(cam2_in)}")
                    print(f"{'='*70}")
